...
var stars = mutableMapOf<Pair<Int, Int>,List<Int>>()
    ...
    for ((columnIndex, policko: Char) in row.withIndex()) {
        ...
        for (rowIndex2 in minRowIndex..maxRowIndex) {
            for (columnIndex2 in minColumnIndex..maxColumnIndex) {
                if (map[rowIndex2][columnIndex2] == '*') {
                    if (stars.containsKey(Pair(rowIndex2, columnIndex2))) {
                        stars[Pair(rowIndex2, columnIndex2)] = stars.getValue(Pair(rowIndex2, columnIndex2)) + listOf(number)
                    } else {
                        stars[Pair(rowIndex2, columnIndex2)] = listOf(number)
                    }
                }
            }
        }
