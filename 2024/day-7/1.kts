...
fun applyOperator(operator: String, acc: Long, numbers: List<Long>, testValue: Long): Boolean {
    ...
    return if (operator == "+") {
        applyOperator("+", acc + numbers.first(), numbers.drop(1), testValue) || applyOperator("*", acc + numbers.first(), numbers.drop(1), testValue)
    } else {
        applyOperator("+", acc * numbers.first(), numbers.drop(1), testValue) || applyOperator("*", acc * numbers.first(), numbers.drop(1), testValue)
    }
}
...
input.filter {
    applyOperator("+", it.second.first(), it.second.drop(1), it.first) || applyOperator("*", it.second.first(), it.second.drop(1), it.first)
}.sumOf { it.first }