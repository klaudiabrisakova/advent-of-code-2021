...
val mapInverse = mapOf(
    "^" to "v",
    "v" to "^"
)

fun moveTwo(first: Pair<Int,Int>, second: Pair<Int, Int>, map: MutableList<MutableList<String>>, where: String): Pair<Int, Int>? {
    val firstMoved = move(first, map, where)
    if (firstMoved != null) {
        val secondMoved = move(second, map, where)
        if (secondMoved == null) {
            move(firstMoved, map, mapInverse.getValue(where))
            return null
        } else {
            return firstMoved
        }
...
//    println("new position $newPosition new sign $newSign")
    ...
    if (newSign == ".") {
        ...
        if (where == "^" || where == "v") {
            if (moveTwo(newPosition,position.first to position.second + 1, map, where) != null) {
                val s = map[position.first][position.second]
                val ns = map[newPosition.first][newPosition.second]
                map[position.first][position.second] = ns
                map[newPosition.first][newPosition.second] = s
...
var rowIndex = newMap.indexOfFirst { it.contains("@") }
var colIndex = newMap[rowIndex].indexOf("@")
